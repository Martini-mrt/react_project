...
function create(type) {
  ...
  const dir = path.join(new URL(import.meta.url).pathname, '../..', 'src', type, name);
...
`);
  ...
  fs.writeFileSync(path.join(dir, `${name}.types.ts`), 
...
`);
  ...
  fs.writeFileSync(path.join(dir, `${name}.scss`), 
`.${baseClass} {
  // styles
}
`);
...
`);
  ...
  console.log(`✅ Компонент "${name}" создан в src/${type}/${name}`);
}